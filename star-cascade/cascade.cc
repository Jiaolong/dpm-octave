...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) { 
  ...
  ofstream logfile;
  logfile.open("log.txt", ios::binary);
      ...
      for (int rx = ceil(padx/2.0); rx < dim[1] - ceil(padx/2.0); rx++) {
        for (int ry = ceil(pady/2.0); ry < dim[0] - ceil(pady/2.0); ry++) {
          ...
          for (; stage < numstages; stage++) {
              double t = MODEL->t[comp][2*stage-1];
            ...
            if (rx == 16 && ry == 25 && stage == 1)
                logfile << "rlevel=" << rlevel <<", comp="<< comp << ", stage=" << stage << ", score=" << score << endl;
          }
