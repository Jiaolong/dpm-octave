...
try
  load([cachedir cls '_' dataset_fg '_' year '.mat']);
catch
  ...
  for i = 1:length(ids);
    ...
    imgname = sprintf(VOCopts.imgpath, ids{i});
    ...
    for j = clsinds(:)'
      ...
      pos(numpos).im      = imgname;%[VOCopts.datadir rec.imgname];
    ...
    numimpos                = numimpos + 1;
    impos(numimpos).im      = imgname;%[VOCopts.datadir rec.imgname];
  ...
  for i = 1:length(ids);
    ...
    if length(clsinds) == 0
      dataid             = dataid + 1;
      numneg             = numneg+1;
      neg(numneg).im     = imgname;%[VOCopts.datadir rec.imgname];
      neg(numneg).flip   = false;
      neg(numneg).dataid = dataid;
    end
  end
  
  save('-mat7-binary', [cachedir cls '_' dataset_fg '_' year '.mat'], 'pos', 'neg', 'impos');
end
