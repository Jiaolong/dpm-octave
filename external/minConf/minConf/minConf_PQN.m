...
if verbose >= 3
   fprintf_flush('Running PQN...\n');
   fprintf_flush('Number of L-BFGS Corrections to store: %d\n',corrections);
   fprintf_flush('Spectral initialization of SPG: %d\n',bbInit);
   fprintf_flush('Maximum number of SPG iterations: %d\n',SPGiters);
   fprintf_flush('SPG optimality tolerance: %.2e\n',SPGoptTol);
   fprintf_flush('SPG progress tolerance: %.2e\n',SPGprogTol);
   fprintf_flush('PQN optimality tolerance: %.2e\n',optTol);
   fprintf_flush('PQN progress tolerance: %.2e\n',progTol);
   fprintf_flush('Quadratic initialization of line search: %d\n',adjustStep);
   fprintf_flush('Maximum number of function evaluations: %d\n',maxIter);
   fprintf_flush('Maximum number of projections: %d\n',maxProject);
end

% Output Log
if verbose >= 2
        fprintf_flush('%10s %10s %10s %15s %15s %15s\n','Iteration','FunEvals','Projections','Step Length','Function Val','Opt Cond');
end
...
if max(abs(funProj(x-g)-x)) < optTol
    if verbose >= 1
        fprintf_flush('First-Order Optimality Conditions Below optTol at Initial Point\n');
    end
    return;
end
...
while funEvals <= maxIter
    ...
    if gtd > -progTol
        if verbose >= 1
            fprintf_flush('Directional Derivative below progTol\n');
        end
        break;
    end
    ...
    while f_new > f + suffDec*g'*(x_new-x) || ~isLegal(f_new)
        ...
        if ~isLegal(f_new) || ~isLegal(g_new)
            if verbose == 3
                fprintf_flush('Halving Step Size\n');
            end
            t = t/2;
        else
            if verbose == 3
                fprintf_flush('Cubic Backtracking\n');
            end
        ...
        if t < temp*1e-3
            if verbose == 3
                fprintf_flush('Interpolated value too small, Adjusting\n');
            end
            t = temp*1e-3;
        elseif t > temp*0.6
            if verbose == 3
                fprintf_flush('Interpolated value too large, Adjusting\n');
            end
            t = temp*0.6;
        end
        ...
        if sum(abs(t*d)) < progTol || t == 0
            if verbose == 3
                fprintf_flush('Line Search failed\n');
            end
            t = 0;
            f_new = f;
            g_new = g;
    ...
    % Output Log
    if verbose >= 2
            fprintf_flush('%10d %10d %10d %15.5e %15.5e %15.5e\n',i,funEvals*funEvalMultiplier,projects,t,f,optCond);
    end

    % Check optimality
        if optCond < optTol
            fprintf_flush('First-Order Optimality Conditions Below optTol\n');
            break;
        end

    if max(abs(t*d)) < progTol
        if verbose >= 1
            fprintf_flush('Step size below progTol\n');
        end
        break;
    end
    ...
    if abs(f-f_old) < progTol
        if verbose >= 1
            fprintf_flush('Function value changing by less than progTol\n');
        end
        break;
    end
    ...
    if funEvals*funEvalMultiplier > maxIter
        if verbose >= 1
            fprintf_flush('Function Evaluations exceeds maxIter\n');
        end
        break;
    end
    ...
    if projects > maxProject
        if verbose >= 1
            fprintf_flush('Number of projections exceeds maxProject\n');
        end
        break;
    end
