...
% Output Log
if verbose >= 2
    fprintf_flush('%10s %10s %15s %15s %15s\n','Iteration','FunEvals','Step Length','Function Val','Opt Cond');
end
...
funEvals = 1;
if verbose >= 1
  fprintf_flush('Initial function value: %15.5e\n', f);
end
...
if isempty(working)
    if verbose >= 1
        fprintf_flush('All variables are at their bound and no further progress is possible at initial point\n');
    end
    return;
elseif norm(g(working)) <= optTol
    if verbose >=1
        fprintf_flush('All working variables satisfy optimality condition at initial point\n');
    end
    return;
end
...
if verbose >= 3
    switch method
        case 'sd'
            fprintf_flush('Steepest Descent\n');
        case 'lbfgs'
            fprintf_flush('L-BFGS\n');
        case 'bfgs'
            fprintf_flush('BFGS\n');
        case 'newton'
            fprintf_flush('Newton\n');
    end
end
...
while funEvals <= maxIter
    ...
    switch(method)
        ...
        case 'newton'
            ...
            if posDef == 0
                ...
                if verbose == 3
                    fprintf_flush('Adjusting Hessian\n');
                end
        ...
        otherwise
            fprintf_flush('Unrecognized Method: %s\n',method);
            break;
    end
    ...
    if gtd > -optTol
        if verbose >= 2
            fprintf_flush('Directional Derivative below optTol\n');
        end
        break;
    end
    ...
    % Output Log
    if verbose >= 2
        fprintf_flush('%10d %10d %15.5e %15.5e %15.5e %15.5e\n',i,funEvals*funEvalMultiplier,t,f,sum(abs(g(working))),norm(g(working)));
    end

    % Check Optimality
    if isempty(working)
        if verbose >= 1
            fprintf_flush('All variables are at their bound and no further progress is possible\n');
        end
        break;
    elseif norm(g(working)) <= optTol
        if verbose >=1
            fprintf_flush('All working variables satisfy optimality condition\n');
        end
        break;
    end
    ...
    if sum(abs(t*d)) < optTol
        if verbose >= 1
            fprintf_flush('Step size below optTol\n');
        end
        break;
    end
    ...
    if abs(f-f_old) < optTol
        if verbose >= 1
            fprintf_flush('Function value changing by less than optTol\n');
        end
        break;
    end
