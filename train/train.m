...
for t = 1:iter
  ...
  if ~cont || t > 1
    ...
    fprintf_flush('saving %d/%d cache entries\n', length(I), num_entries);
    ...
    if warp
      ...
      for tt = 1:t
        fprintf_flush('positive loss before: %f, after: %f, ratio: %f\n', ...
                pos_loss(tt,1), pos_loss(tt,2), pos_loss(tt,2)/pos_loss(tt,1));
      end
      fprintf_flush('Foreground latent estimation took %.4f seconds\n', th);

      if t > 1 && pos_loss(t,2)*0.99999 > pos_loss(t,1)
        fprintf_flush('warning: pos loss went up\n');
        keyboard;
      end
  ...
  for tneg = 1:negiter
    fprintf_flush('%s iter: %d/%d, neg iter %d/%d\n', ...
    ...
    if datamine
      ...
      if randneg
        ...
        fprintf_flush('complete: %d, negative loss of old model: %f\n', ...
                complete, neg_loss(tneg,1));
        for tt = 2:tneg
          cache_val = cache(tt-1,4);
          full_val = cache(tt-1,4)-cache(tt-1,1) + neg_loss(tt);
          fprintf_flush('obj on cache: %f, obj on full: %f, ratio %f\n', ...
                  cache_val, full_val, full_val/cache_val);
        end
        fprintf_flush('Data mining took %.4f seconds\n', th);
      ...
      fprintf_flush('\nFilter usage stats:\n');
      for i = 1:model.numfilters
        fprintf_flush('  filter %d got %d/%d (%.2f%%) negatives\n', ...
                i, fusage(i), num_examples_added, 100*fusage(i)/num_examples_added);
      end

      if ~datamine
        fprintf_flush('Data mining convergence condition met.\n');
      ...
      fprintf_flush('Skipping data mining iteration.\n');
      fprintf_flush('The model has not changed since the last data mining iteration.\n');
      datamine = true;
    end
    ...
    %[[[ Optimize the convex slave problem
      ...
      th = toc(th);
      fprintf_flush('Parameters optimized in %.4f seconds\n', th);
      ...
      save('-mat7-binary', [conf.paths.model_dir model_name '.mat'], 'model');

      % Output history of the objective function value on the cache
      cache(tneg,:) = [nl pl rt nl+pl+rt];
      for tt = 1:tneg
        fprintf_flush('Cache objective: bg: %.6f, fg: %.6f, reg: %.6f, total: %.6f\n', ...
                cache(tt,1), cache(tt,2), cache(tt,3), cache(tt,4));
      end
      fprintf_flush('Finished training %s (C = %.4f)\n', model_name, C);
    %[[[
    ...
    %[[[ Maintain the feature vector cache
      ...
      fprintf_flush('cache byte limit: %d\nnot-mined size: %d\ncapacity: %d\n', ...
      ...
      fprintf_flush('num keep: %d (entries) %d (examples) based on max byte limit\n', ...
      ...
      fprintf_flush('num keep: %d (entries) %d (examples) based on max num examples\n', ...
      ...
      fprintf_flush('cached %d (%d) positive and %d (%d) negative examples (entries)\n', ...
      ...
      fprintf_flush('%d foreground support vectors\n', num_sv);
      I = find((info.is_belief == 0)&(info.is_mined == 1)& ...
               (info.is_unique == 1)&(info.margins < 0.000001));
      num_sv = size(unique([info.dataid(I) info.scale(I) info.x(I) info.y(I)], 'rows'), 1);
      fprintf_flush('%d background support vectors\n', num_sv);
    %]]]
...
loss = 0;
for i = 1:numpos
  fprintf_flush('%s %s: iter %d: warped positive: %d/%d\n', ...
...
for i = 1:batchsize:numpos
  ...
  parfor k = 1:thisbatchsize
    ...
    for b = 1:num_boxes
      ...
      if ~isempty(bs)
        fprintf_flush('%s (%d: comp %d  score %.3f)\n', msg, b, bs(1,end-1), bs(1,end));
      else
        fprintf_flush('%s (%d: no overlap)\n', msg, b);
      end
    end
    model_dp = [];
  end
...
for i = 1:batchsize:numneg
  ...
  parfor k = 1:thisbatchsize
    j = inds(i+k-1);
    fprintf_flush('%s %s: iter %d/%d: hard negatives: %d/%d (%d)\n', ...
  ...
  for k = 1:thisbatchsize
    ...
    if byte_size >= maxsize || num_examples >= max_num_examples
      if num_examples >= max_num_examples
        fprintf_flush('reached example count limit\n');
      else
        fprintf_flush('reached cache byte size limit\n');
      end
      complete = 0;
      break;
    end
...
if s > 0
  while true
    try
      ...
      fprintf_flush('Ugg! Something bad happened. Trying again in 10 seconds...\n');
      pause(10);
    end
  end
end
